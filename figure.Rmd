...
# tdf <- test %>% select(columns) %>% mutate(SITE_ID = factor(SITE_ID,)) %>% mutate(Continent = factor(Continent, ordered=TRUE)) %>% mutate(SEX = factor(SEX)) %>% mutate(DX_GROUP = factor(DX_GROUP)) %>% group_by(SITE_ID) 
